...
@interface CJServiceController ()<UITableViewDataSource,UITableViewDelegate>
{
...
@implementation CJServiceController
...
- (void)viewDidLoad
{
    ...
    self.navigationItem.title = @"专业服务";
...
-(void)getDate {
    [CJRequestFormat getMobileGoodWithType:GoodsService finished:^(ResponseStatus status, NSString *response) {
        if (status == 0) {
            //            NSLog(@"%@",response);
