...
@interface CBookController ()<UITableViewDataSource,UITableViewDelegate>
{
...
@implementation CBookController
...
- (void)viewDidLoad
{
    ...
    self.navigationItem.title = @"书籍分享";
...
-(void)getDate {
    [CJRequestFormat getMobileGoodWithType:GoodsBook finished:^(ResponseStatus status, NSString *response) {
        if (status == 0) {
            //            NSLog(@"%@",response);
