...
@interface CJHotGoodController ()<UITableViewDelegate,UITableViewDataSource>
{
...
@implementation CJHotGoodController
...
- (void)viewDidLoad
{
    ...
    self.navigationItem.title = @"热门礼物";
...
-(void)getDate {
    [CJRequestFormat getMobileGoodWithType:GoodsHot finished:^(ResponseStatus status, NSString *response) {
        if (status == 0) {
            //            NSLog(@"%@",response);
